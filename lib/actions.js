...
function Actions(config, cwd) {
  ...
  var setttingsJsonPath = path.resolve(this.cwd, ".deploy/settings.json");
  if(fs.existsSync(setttingsJsonPath)) {
    console.log( ("Meteor Settings read from "+setttingsJsonPath).bold.green );
...
Actions.init = function() {
  ...
  if ( fs.existsSync("./.deploy") )
    console.error("MUP .deploy folder already exists".bold.red);
  else
    fs.mkdirSync("./.deploy");

  // Resolve and create default config files:
  var destMupJson = path.resolve(".deploy/default.json");
  var destSettingsJson = path.resolve(".deploy/settings.json");

  if(fs.existsSync(destMupJson) || fs.existsSync(destSettingsJson)) {
    console.error('MUP configuration files already exist'.bold.red);
    process.exit(1);
  }
  ...
  console.log('MUP deployment configuration files added to project under .deploy'.bold.green);
