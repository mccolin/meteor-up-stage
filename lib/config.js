...
exports.read = function(targetEnv) {
  var mupJsonBasename = targetEnv+".json";
  var mupJsonPath = path.resolve(".deploy/"+mupJsonBasename);
  console.log( ("Deployment configuration read from "+mupJsonPath).bold.green );
  ...
  if(fs.existsSync(mupJsonPath)) {
    ...
    console.error( ("Deployment configuration file "+mupJsonPath+" does not exist").red.bold );
    helpers.printHelp();
    process.exit(1);
  }
};
